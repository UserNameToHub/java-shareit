...
public interface BookingRepository extends BaseRepository<Booking, Long> {
    ...
    List<Booking> findAllByBookerIdAndStartDateIsAfter(Long id, LocalDateTime dateTime, Sort sort);

    @Query("select bk from Booking as bk " +
            "join fetch bk.booker as bkr " +
            "where bkr.id = :id and :dateTime between bk.startDate and bk.endDate")
    List<Booking> findAllCurrentByBookerId(Long id, LocalDateTime dateTime, Sort sort);
    ...
    List<Booking> findAllCurrentByItemOwnerId(Long id, LocalDateTime dateTime, Sort sort);
    ...
    @Query("select new ru.practicum.shareit.item.dto.ItemBookingGettingTo(bk.id, bk.booker.id) " +
            "from Booking as bk " +
            "where bk.item.id = :id and bk.status = 'APPROVED' and bk.startDate < :dateTime " +
            "order by bk.startDate desc")
    List<ItemBookingGettingTo> findLastById(@Param("id") Long id,
    ...
    List<ItemBookingGettingTo> findNextById(@Param("id") Long id,
                                            @Param("dateTime") LocalDateTime dateTime,
                                            @Param("limit") Pageable limit);
